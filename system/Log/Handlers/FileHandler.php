...
class FileHandler extends BaseHandler
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->filePermissions = $config['filePermissions'] ?? 0777;
    ...
    {
        //echo "mensahe errir".$message;
        $filepath = $this->path . 'log-' . date('Y-m-d') . '.' . $this->fileExtension;
        //echo "mensahe errir".$filepath;

        $msg = '';

        if (! is_file($filepath)) {
            $newfile = true;
            //echo "mensajeError newfile".$newfile;
        ...
        if (! $fp = fopen($filepath, 'a+')) {
            //echo "mensajeError return false";
            return false;
        }
        ...
        //echo "mensajeError fin arch";

        if (isset($newfile) && $newfile === true) {
            //echo "mensajeError fin permissions";
            chmod($filepath, $this->filePermissions);
        }
        
        //echo "mensajeError fin pro";
